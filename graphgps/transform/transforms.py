...
def pre_transform_in_memory(dataset, transform_func, show_progress=False):
    ...
    dataset.data, dataset.slices = dataset.collate(data_list) # data.split disappear



def pre_transform_in_memory_glora(dataset, transform_func, show_progress=False):
    """Pre-transform already loaded PyG dataset object.
    ...
    Implementation is based on torch_geometric_utils.data.in_memory_dataset.copy
    ...
    for i in tqdm(range(len(dataset)),
                  ...
                  mininterval=10,
                  miniters=len(dataset) // 20):
        transformed_data = transform_func(dataset.get(i))
