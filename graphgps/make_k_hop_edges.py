...
def make_k_hop_edges(dataset, K, format, name):
    ...
    dataset.data.edge_attr = labels  # [graph 1 hop1 node labels, graph 1 hop2 node labels, graph2 hop1 node labels, graph 2 hop 2 node labels]
    dataset.slices['edge_index'] = dataset.slices['edge_attr'] = ei_slices # ei: edge index

    print('Checking correct conversion...')
    count = 0
    for i in tqdm(range(len(dataset))):
        if dataset.get(i).edge_attr==None or not torch.equal(dataset.get(i).edge_attr.float(),
...
def compute_k_hop_edges(dataset, K, filedir, format, name, extra):
    ...
    # if dataset.slices == None: # only one graph/
