...
def make_fa_edges(dataset, K, format, name):
    ...
    list_of_K = [1, -10]  # add -10 to list of K for FA edges indices
    ...
    if not all(file_exists):  # checks all files are there
        ...
        compute_k_hop_edges(dataset, K, filedir, format, name, extra)  # if they don't, make them, here K is always 1

        if not exists(join(filedir, "%s-%s%s_k=%02d.pt" % (format, name, extra, -10))):
            compute_fa_edges(dataset, filedir, format, name, extra)
...
def compute_fa_edges(dataset, filedir, format, name, extra):
    ...
    edge_indices = dataset.data.edge_index
    slices = dataset.slices['edge_index']  # used to slice the graph
    idxs = [[]] # only need for fully adjacency
    for i in tqdm(range(len(slices) - 1)):
        edge_index = edge_indices[:, slices[i]:slices[i + 1]]
        # number of nodes
