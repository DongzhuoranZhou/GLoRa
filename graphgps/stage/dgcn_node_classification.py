...
from graphgps.layer.dgcn_conv import DGCNConv
...
class DGCN_node_classification(torch.nn.Module):
    ...
    def __init__(self, dim_in: int, dim_out: int, num_layers: int,
        ...
        self.Conv = nn.Conv1d(dim_out*3, dim_out, kernel_size=1)
        ...
        # Initialize linear layers and biases
        self.lin_layers = torch.nn.ModuleList()
        self.biases = torch.nn.ParameterList()

        self.lin_layers.append(torch.nn.Linear(dim_in, dim_out, bias=False))
        self.biases.append(nn.Parameter(torch.Tensor(1, dim_out)))

        for _ in range(1, self.k):
            self.lin_layers.append(torch.nn.Linear(dim_out * 3, dim_out, bias=False))
            self.biases.append(nn.Parameter(torch.Tensor(1, dim_out)))
    ...
    def forward(self, batch) -> torch.FloatTensor:
        ...
        edge_in, edge_out = batch.edge_in, batch.edge_out
        in_w, out_w = batch.in_weight, batch.out_weight
        # x = self.lin1(x)
...
stage_dict = {
    'dgcn_gnn': DGCN_node_classification,
}
