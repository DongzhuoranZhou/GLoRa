...
class DRewGNNStage(nn.Module):
    ...
    def forward(self, batch):
        ...
        k is the hop distance, t is the num of layers, in paper t is l.
        """
        ...
        for t in range(self.num_layers):
            ...
            alpha = self.alpha_t[t] if cfg.agg_weights.use else torch.ones(
                len(k_neighbourhoods))  # learned weights or equal weights (default equal)
            alpha = F.softmax(alpha, dim=0)
            alpha = alpha if cfg.agg_weights.convex_combo else alpha * len(
                k_neighbourhoods)  # convex combination or not (default unity weights, no CC)
            for i, k in enumerate(k_neighbourhoods):
                if A(k).shape[1] > 0:  # iff there are edges of type k
                    ...
                    try:
                        ...
                        print('Error in layer %d, k=%d' % (t, k))   # this is expected for VOC and COCO
