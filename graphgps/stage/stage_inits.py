...
def init_DRewGCN(model, dim_in, dim_out, num_layers, skip_first_hop=False):
    ...
    for t in range(t0, num_layers):
        ...
        if use_weights: alpha_t.append(
            torch.nn.Parameter(torch.ones(len(k_neighbourhoods)), requires_grad=True))  # unity init
    model.W_kt = nn.ModuleDict(W_kt)
    if use_weights: model.alpha_t = nn.ParameterList(alpha_t)
    return model

# def init_DRewGAT(model, dim_in, dim_out, num_layers, skip_first_hop=False):
#     """The (nu)DRew-GAT param initialiser, used for drew_gnn"""
#     model.num_layers = num_layers
#     W_t = []
#     t0 = 1 if skip_first_hop else 0
#     for t in range(t0, num_layers):
#         d_in = dim_in if t == 0 else dim_out
#         W_t.append(GATConv(d_in, dim_out, heads=1, concat=False))  # GAT layers as specified
#
#     model.W_t = nn.ModuleList(W_t)
#     return model
...
def init_SP_GCN(model, dim_in, dim_out, num_layers, max_k=None):
    ...
    if cfg.max_graph_diameter <= model.max_k:
        print(
            "Warning: max_graph_diameter = %d; <= max_k, so setting max_k to max_graph_diameter" % cfg.max_graph_diameter)
