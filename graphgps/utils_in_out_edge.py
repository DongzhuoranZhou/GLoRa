...
def directed_features_in_out(edge_index, size: int, edge_weight=None):
    ...
    edge_in = torch.from_numpy(np.vstack((A_in.row, A_in.col)))
    edge_out = torch.from_numpy(np.vstack((A_out.row, A_out.col)))

    in_weight = torch.from_numpy(A_in.data)
    out_weight = torch.from_numpy(A_out.data)
    index_undirected = to_undirected(edge_index)

    device = edge_index.device
    return index_undirected.to(device), edge_in.to(device), in_weight.to(device), edge_out.to(device), out_weight.to(
        device)
